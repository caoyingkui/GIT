...
/**
 * Created by kvirus on 2019/6/21 7:49
 ...
 * |   *******        **         **     **
 */
public class ISwitchStatement extends InsertHash{
    ...
    public ISwitchStatement(SourceCodeChange change) {
        assert (change instanceof Insert || change instanceof Delete) &&
                change.getChangedEntity().getType() == JavaEntityType.SWITCH_STATEMENT;

        hashes = new int[5];
        hashes[ACTION] = typeHash(change);
        hashes[STATEMENT] = getCode(ASTNode.SWITCH_STATEMENT);
    ...
    public boolean equals(StatementHash hash) {
        if (!(hash instanceof ISwitchStatement)) return false;
