...
public class Generators extends Registry<String, TreeGenerator, Register> {
    ...
    public TreeContext getTree(String file, int kind) throws UnsupportedOperationException, IOException {
        TreeGenerator p = get(file);
        if (p == null)
            throw new UnsupportedOperationException("No generator found for file: " + file);
        return p.generateFromFile(file, kind);
    }

    public TreeContext getTree(String generator, String file, int kind) throws UnsupportedOperationException, IOException {
        for (Entry e : entries)
            if (e.id.equals(generator))
                return e.instantiate(null).generateFromFile(file, kind);
